...
class ProfileActivity : BaseFullscreenActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        b.clientNameText.text = session?.displayName?.takeIf { it.isNotBlank() } ?: "ADWIO User"
        b.usernameText.text = session?.username?.takeIf { it.isNotBlank() } ?: "M3U User"
        b.statusText.text = session?.status?.takeIf { it.isNotBlank() } ?: "Active"
        b.sourceTypeText.text = if (session?.server?.id == "m3u") "M3U" else "Xtream Codes"
        b.expiryText.text = formatEpoch(session?.expiresAt)
        b.createdText.text = formatEpoch(session?.createdAt)
        b.activeConnectionsText.text = session?.activeConnections?.takeIf { it.isNotBlank() } ?: "—"
        b.maxConnectionsText.text = session?.maxConnections?.takeIf { it.isNotBlank() } ?: "—"
    ...
    private fun formatEpoch(raw: String?): String {
        ...
        return runCatching {
            SimpleDateFormat("dd MMM yyyy", Locale.getDefault()).format(Date(seconds * 1000L))
        }.getOrDefault(raw)
    }
}
