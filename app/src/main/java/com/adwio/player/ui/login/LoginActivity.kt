...
import com.adwio.player.data.M3uXtreamResolver
...
class LoginActivity : BaseFullscreenActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!editId.isNullOrBlank()) {
            PlaylistStore(this).find(editId)?.let { profile ->
                ...
                if (profile.serverId.startsWith("m3u", ignoreCase = true)) {
                    b.m3uRadio.isChecked = true

                    /*
                     * Generic M3U profiles store the full M3U URL.
                     * Bridged M3U profiles store the base host + credentials,
                     * so rebuild a normal get.php URL for editing.
                     */
                    val m3uValue =
                        if (profile.serverId == "m3u_xtream" &&
                            profile.username.isNotBlank() &&
                            profile.password.isNotBlank()
                        ) {
                            "${profile.serverUrl.trimEnd('/')}/get.php" +
                    ...
                    b.m3uUrlInput.setText(m3uValue)
                } else {
    ...
    private fun connect() {
        ...
        if (name.isBlank()) {
            b.errorText.text = "Enter playlist name"
            return
        }
    ...
    private fun connectXtream(name: String) {
        ...
        if (server.isBlank() || username.isBlank() || password.isBlank()) {
            b.errorText.text = "Enter server URL, username and password"
            return
        }
        ...
        lifecycleScope.launch {
            ...
            if (session == null) {
                b.errorText.text = "Unable to connect. Check playlist details."
                return@launch
            }
    ...
    private fun connectM3u(name: String) {
        ...
        if (url.isBlank()) {
            b.errorText.text = "Enter M3U URL"
            return
        }
        ...
        lifecycleScope.launch {
            /*
             * PRIMARY PATH:
             * If this is an Xtream-generated M3U URL, use the exact Xtream API
             * path already proven to load Live / Movies / Series correctly.
             *
             * The account remains labelled M3U in the UI via server id
             * "m3u_xtream", while internally content uses XtreamClient.
             */
            val xtreamCreds = M3uXtreamResolver.parse(url)
            ...
            if (xtreamCreds != null) {
                ...
                if (authenticated != null) {
                    getSharedPreferences("adwio_m3u", MODE_PRIVATE)
                        ...
                        .putString(
                            "active_epg",
                            b.epgUrlInput.text?.toString()?.trim().orEmpty()
                        )
                        .apply()

                    val bridged = authenticated.copy(
                        server = ServerHost(
                            id = "m3u_xtream",
                            name = authenticated.server.name,
                            baseUrl = xtreamCreds.baseUrl
                        ),
                        displayName = name
                    )

                    setLoading(false)
                    saveAndOpen(name, bridged)
                    return@launch
                }
            }
            ...
            /*
             * FALLBACK:
             * True/raw M3U URLs that do not contain Xtream credentials continue
             * through the normal M3U parser.
             */
            ...
            saveAndOpen(
                ...
                Session(
                    username = "",
                    password = "",
                    server = ServerHost("m3u", host, url),
                    expiresAt = null,
                    status = "Active",
                    displayName = name
                )
